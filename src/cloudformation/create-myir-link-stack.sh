...
create_cloudformation_stack () {
  ...
  roleName="stc-myir-link-cloudformation-role-${ENVIRONMENT_TYPE}"
  ...
  --role-name "$roleName" \
  --policy-name "stc-myir-link-cloudformation-policy-${ENVIRONMENT_TYPE}" \
  ...
  --stack-name "stc-myir-link-api-${ENVIRONMENT_TYPE}" \
  --template-body "${API_STACK_TEMPLATE}" \
  --capabilities CAPABILITY_IAM CAPABILITY_NAMED_IAM \
  --role-arn "$roleArn" \
  --change-set-name "stc-myir-link-api-${ENVIRONMENT_TYPE}-changeset" \
  ...
  echo "Cloudformation stc-myir-link-api-${ENVIRONMENT_TYPE} stack created with the id $stackId"
  ...
  echo "Cloudformation stc-myir-link-pipeline-${ENVIRONMENT_TYPE} stack created with the changeSet $pipelineChangeSetId"
  ...
  echo "Executing the change set of the stc-myir-link-pipeline-${ENVIRONMENT_TYPE} stack"
  ...
  aws --profile="${AWS_CLI_PROFILE}" cloudformation wait stack-create-complete --stack-name "stc-myir-link-pipeline-${ENVIRONMENT_TYPE}"
